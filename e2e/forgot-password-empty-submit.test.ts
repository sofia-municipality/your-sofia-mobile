...
describe('Forgot password screen', () => {
  ...
  beforeEach(async () => {
    ...
    await waitFor(element(by.id('loginForgotPasswordLink')))
      .toBeVisible()
      .withTimeout(10000)
    await element(by.id('loginForgotPasswordLink')).tap()
  })

  it('shows a validation alert when submitting with an empty email', async () => {
    await waitFor(element(by.id('forgotPasswordSubmitButton')))
      .toBeVisible()
      .withTimeout(10000)
    ...
    await expect(
      element(by.text('Въведете имейл адреса си, за да получите инструкции за нова парола.'))
    ).toBeVisible()
